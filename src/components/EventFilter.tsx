        ...
        <div className="flex flex-row justify-start flex-wrap gap-10 mt-12">
          {currentFilter.map((item, index) => (
            <CustomButton
              ...
              onclick_func={() => setCurrent(item.text)}
            />
          ))}
        </div>
            ...
            .map((item) =>
              item.data.map((item, index) => (
                <Link
                  ...
                  key={index}
                  className="flex flex-col gap-x-5 p-1 shadow-custom-sd rounded-lg max-w-[300px] bg-white relative hover:scale-105 duration-300"
                >
                  <img
                    src={item.img}
                    alt="image"
